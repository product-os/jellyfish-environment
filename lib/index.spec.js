...
ava('isProduction() returns true if the node environment is `production`', (test) => {
	const environment = require('./index').getEnvironment({
		NODE_ENV: 'production'
	})
...
// Ensure that no variable groups or public functions are mistakenly added/removed.
ava('output has all expected keys', (test) => {
	const environment = require('./index').getEnvironment({})
	test.deepEqual(Object.keys(environment), [
		'actions',
		'aws',
		'database',
		'fileStorage',
